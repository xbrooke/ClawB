...
const tabs: PlatformTab[] = [
  ...
  { id: "weixin", label: "微信", Icon: Smartphone },
];
...
function WeixinPanel({ onInstall }: { onInstall: () => void }) {
  return (
    <div className="glass-card" style={{ display: "flex", flexDirection: "column", gap: 20, padding: 24 }}>
      <div>
        ...
        <p style={{ fontSize: 13, color: "var(--text-secondary)", margin: 0, lineHeight: 1.6 }}>
          点击下方按钮安装微信插件，安装过程中会弹出二维码，请使用手机微信扫码绑定。
        </p>
      </div>
...
export function PlatformsPage() {
  ...
  const handleInstallWeixin = () => {
    ...
    installWeixinPlugin(
      ...
      (result) => {
        setInstallStatus(result === "success" ? "success" : "failed");
      }
    );
  };
  ...
  return (
    <div style={{ padding: "48px 40px 60px", maxWidth: 680, width: "100%", margin: "0 auto", display: "flex", flexDirection: "column", gap: 32 }}>
      ...
      {activeTab === "weixin" && (
        <WeixinPanel onInstall={handleInstallWeixin} />
      )}

      {activeTab === "feishu" && <FeishuPanel />}

      {activeTab === "weixin" && installStatus !== "idle" && (
          ...
          {installOutput.length > 0 && (
            <div style={{
              ...
              {installOutput.map((line, i) => (
                <div key={i} style={{ lineHeight: 1.8 }}>{line}</div>
              ))}
            </div>
          )}
